...
if __name__=="__main__":
	...
	triangles = [triangle]
	cameraPos = np.array([3.0,0.0,0.0,1.0])
	...
	for count in range(how_many_cycles):
		...
		print("Shitoof")
		print("The triangle:\n")
		print(triangles_sorted[0].point_matrix)
		print("\n\n")
		shit_count = 1
		for triangle in triangles_sorted:
			...
			for p in range(4):
				...
				while amount_of_new_triangles > 0:
					...
					print("Current triangles in loop: ")
					...
					print("All current triangles: ")
					for oof in current_triangle_list:
						print(oof.point_matrix)
					...
					if p == 1:
						print("p == 1")
						print(cur_triangle)
						how_many_to_add, appended_triangles = cliptriangleagainstplane(np.array([0,(screenheight/2)-1,0,0]), np.array([0,-1,0,0]), cur_triangle, debug=True)
					if p == 2:
						how_many_to_add, appended_triangles = cliptriangleagainstplane(np.array([-screenwidth/2+1,0,0,0]), np.array([1,0,0,0]), cur_triangle)
					if p == 3:

						how_many_to_add, appended_triangles = cliptriangleagainstplane(np.array([screenwidth/2-1,0,0,0]), np.array([-1,0,0,0]), cur_triangle, debug=True)
