...
if __name__=="__main__":
	...
	screenheight = 3000
	when_to_draw = 1000
	sun_direction = normalise_vector(np.array([0,1,0,0]))
	...
	while True:
		...
		triangles_sorted = sorted(triangles_to_sort, key=lambda x: (x.point_matrix[0][2]+x.point_matrix[1][2]+x.point_matrix[2][2])/3.0, reverse=False) # first point and the z coordinate
		...
		for triangle in triangles_sorted:
			...
			for triangle in current_triangle_list:
				...
				if triangle_count % when_to_draw == 0:
					turtle.update()
				color = getcolor(triangle, sun_direction)
				print(color)
				drawtriangle(turtleobj, triangle, color)
