...
def do_camera_transform_for_triangle(triangle: Triangle, camera: Camera):
	for i in range(3):
		triangle.point_matrix[i] = camera.transform_point(triangle.point_matrix[i])
	return triangle
...
if __name__=="__main__":
	...
	triangles = [triangle]
	cameraPos = np.array([0.0,0.0,0.0,0.0])
	...
	for triangle in triangles:
		...
		triangle = do_camera_transform_for_triangle(triangle, camera)
	
		# Clip against near plane:
		# here the near plane is assumed to be at distance 1 from the camera

		number_of_clipped_triangles, out_triangles = cliptriangleagainstplane(np.array([0,0,1,1]), np.array([0,0,1,0]), triangle)
		...
		for i in range(number_of_clipped_triangles):
			...
			number_of_clipped_triangles[i] = project_triangle_from_2d_to_3d(number_of_clipped_triangles[i], projection_matrix)



			# offset into normalised space

			voffset = np.array([1,0,0,0])

			for k in range(3):
				number_of_clipped_triangles[i].point_matrix[0] = number_of_clipped_triangles[i].point_matrix[0] + voffset
				

			for j in range(3):
				number_of_clipped_triangles[i].point_matrix[j][0] = number_of_clipped_triangles[i].point_matrix[j][0] * (0.5 * screenwidth)
				number_of_clipped_triangles[i].point_matrix[j][1] = number_of_clipped_triangles[i].point_matrix[j][1] * (0.5 * screenheight)


			triangles_to_sort.append(number_of_clipped_triangles[i])
	...
	for triangle in triangles_sorted:
		...
		for p in range(4):
			...
			while amount_of_new_triangles > 0:
				...
				if p == 0:
					how_many_to_add, appended_triangles = cliptriangleagainstplane(np.array([0,0,0]), np.array([0,1,0]), cur_triangle)
				if p == 1:
					how_many_to_add, appended_triangles = cliptriangleagainstplane(np.array([0,screenheight-1,0]), np.array([0,-1,0]), cur_triangle)
				if p == 2:
					how_many_to_add, appended_triangles = cliptriangleagainstplane(np.array([0,0,0]), np.array([1,0,0]), cur_triangle)
				if p == 3:
					how_many_to_add, appended_triangles = cliptriangleagainstplane(np.array([screenwidth-1,0,0]), np.array([-1,0,0]), cur_triangle)
